...
CONFIG = {
    ...
    'ckpt_dir':      CKPT_DIR,
    'eer_plot_path':  os.path.join(OUTPUT_DIR, 'far_frr_eer_curve.png'),
...
def save_far_frr_eer_plot(metrics, path):
    thresholds = metrics['thresholds']
    far        = metrics['far']
    frr        = metrics['frr']
    eer        = metrics['eer']
    eer_thresh = metrics['eer_thresh']

    fig, ax = plt.subplots(figsize=(8, 6))

    ax.plot(thresholds, far, label='FAR (False Accept Rate)', color='#e74c3c', linewidth=2)
    ax.plot(thresholds, frr, label='FRR (False Reject Rate)', color='#2980b9', linewidth=2)

    # EER point
    ax.axvline(eer_thresh, color='#27ae60', linestyle='--', linewidth=1.5,
               label=f'EER threshold = {eer_thresh:.4f}')
    ax.scatter([eer_thresh], [eer], color='#27ae60', zorder=5, s=80)
    ax.annotate(
        f'EER = {eer*100:.2f}%',
        xy=(eer_thresh, eer),
        xytext=(eer_thresh + 0.05, eer + 0.04),
        fontsize=10,
        color='#27ae60',
    ...
    ax.set_title('FAR / FRR / EER Curve', fontsize=14, fontweight='bold')
    ...
    plt.close(fig)
    print(f'FAR/FRR/EER curve saved: {path}')
# ────────────────────────────────
#  MAIN
# ────────────────────────────────
...
def train(processed_dir):
    ...
    save_far_frr_eer_plot(metrics, CONFIG['eer_plot_path'])
