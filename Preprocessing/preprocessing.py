...
Pipeline:
  ...
  6. Otsu Segmentation
  7. CLAHE enhancement  (boosts low-contrast NIR vein structure)
  8. Sato Vesselness Filter  (pure NumPy/SciPy, no DLL issues)
  9. Tight crop ROI + Resize to TARGET_SIZE
...
# -- Sato filter -------------------------------------------------------------
Sato_SCALE_MIN  = 4    # minimum vein thickness to detect (pixels)
Sato_SCALE_MAX  = 4    # maximum vein thickness to detect (pixels)
...
# -- CLAHE ---------------------------------------------------------------------
CLAHE_CLIP      = 1.5  # clip limit for CLAHE (boosts NIR contrast before Sato)
...
def visualize_pipeline(stages):
    """
    Display 6 pipeline stages in a 2-row x 3-col grid on a dark background.

    Row 1: Original | Blur+Crop | Wrist & Fingers removed
    Row 2: CLAHE    | Sato      | 128x128 Normalized
    """
    assert len(stages) == 6, f"Expected 6 stages, got {len(stages)}"

    import matplotlib.pyplot as plt  # lazy — not needed on Pi server
    fig, axes = plt.subplots(2, 3, figsize=(3 * 5, 2 * 4.2))
...
if __name__ == "__main__":
    ...
    print("\n[Step 7] CLAHE Enhancement  (on raw metacarpal — no hard edges)")
    img_clahe = apply_clahe(img_metacarpal,
    ...
    stages = [
        ...
        (img_metacarpal, "3. Wrist & Fingers Removed"),
        (img_clahe,      "4. CLAHE"),
        (img_Sato,       "5. Sato"),
        (img_save,       f"6. {TARGET_SIZE}x{TARGET_SIZE} Normalized"),
    ]
